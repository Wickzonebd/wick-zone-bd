    ...
    <main className={`auth-page auth-page-${mode}`}>
      <section className="auth-card">
        ...
        <form className="auth-form" onSubmit={submit}>
          {mode === "register" && <div className="field"><label>{t("auth.fullName")}</label><div className="input-wrap"><UserRound size={20} /><input className="input with-icon" value={fullName} onChange={(event) => setFullName(event.target.value)} autoComplete="name" required /></div></div>}
          {mode !== "reset" && <div className="field"><label>{t("auth.email")}</label><div className="input-wrap"><Mail size={20} /><input className="input with-icon" type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" required /></div></div>}
