...
import { Activity, BadgeCheck, BriefcaseBusiness, Check, CircleDollarSign, FileCheck2, Gift, ImagePlus, LayoutDashboard, LockKeyhole, Megaphone, Newspaper, Palette, Plus, RefreshCw, Search, Settings, ShieldCheck, Sparkles, Trash2, UploadCloud, UserRound, UsersRound, WalletCards, X } from "lucide-react";
...
export function AdminClient() {
  ...
  const setMembershipAccess = async (item: AdminUser, active: boolean) => {
    ...
    if (active) {
      const { error: badgeError } = await supabase.rpc("admin_set_user_badge", {
        p_user_id: item.id,
        p_badge: "Verified",
        p_reason: "Verified badge for complimentary full access",
      });
      setMessage(badgeError ? `Free access was granted, but badge failed: ${badgeError.message}` : `${item.full_name} now has free full access. No payment is required.`);
      ...
      setMessage(badgeError ? `Access was revoked, but badge failed: ${badgeError.message}` : "Member access revoked and Verified badge removed.");
    } else {
  ...
  if (authLoading) return <AppShell hidePrimaryNav><main className="admin-shell"><LoadingCards count={5} /></main></AppShell>;
  ...
  return <AppShell hidePrimaryNav><main className="admin-shell"><section className="admin-hero"><div className="admin-hero-main"><div className="admin-hero-icon"><ShieldCheck size={28} /></div><div><span className="admin-kicker light">WICK ZONE CONTROL CENTER</span><h1>{t("admin.title")}</h1><p>{t("admin.subtitle")}</p></div></div><div className="admin-hero-status"><span className="admin-live-dot" /><div><strong>Administrator · Full access</strong><small>No activation fee. Protected actions are checked by Supabase.</small></div></div></section>
    ...
    {loading ? <LoadingCards count={5} /> : error ? <ErrorState message={t("common.error")} /> : <>
      {tab === "dashboard" && stats && <div className="admin-dashboard"><div className="admin-access-note"><ShieldCheck size={22} /><div><strong>This Admin account never pays an activation fee.</strong><span>You have full protected access and can control customer access, money, jobs, proofs, services, content and site settings here.</span></div></div><div className="admin-command-grid">
        ...
        <button className="admin-command-card" onClick={() => setTab("content")}><span className="admin-command-icon"><Megaphone size={21} /></span><strong>Services & Content</strong><small>Banners, services, projects and ticker</small></button>
