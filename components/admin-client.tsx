...
import { Activity, BadgeCheck, BriefcaseBusiness, Check, CircleDollarSign, CreditCard, FileCheck2, FileText, Gift, ImagePlus, LayoutDashboard, Link2, LockKeyhole, Megaphone, MessageSquareText, Newspaper, Palette, Plus, RefreshCw, Search, Settings, ShieldCheck, ShoppingBag, ShoppingCart, Sparkles, Store, Trash2, Trophy, UploadCloud, UserRound, UsersRound, WalletCards, X } from "lucide-react";
...
export function AdminClient() {
  ...
  const saveSettings = async (event: FormEvent) => {
    ...
    if (!Number.isInteger(settingsDraft.referralRewardCoins) || settingsDraft.referralRewardCoins < 0) { setMessage("Referral reward must be a whole number of coins."); return; }
    if (!Number.isInteger(settingsDraft.coinsPerCurrencyUnit) || settingsDraft.coinsPerCurrencyUnit < 1) { setMessage("Coins per currency unit must be at least 1."); return; }
    if (!Number.isInteger(settingsDraft.minimumCoinExchange) || settingsDraft.minimumCoinExchange < 1) { setMessage("Minimum coin exchange must be at least 1 coin."); return; }
  ...
  return <AppShell hidePrimaryNav adminNavigation={adminNavigation}><main className="admin-shell"><section className="admin-hero"><div className="admin-hero-main"><div className="admin-hero-icon"><TaskoraMark size={34} /></div><div><span className="admin-kicker light">TASKORA CONTROL CENTER</span><h1>{t("admin.title")}</h1><p>{t("admin.subtitle")}</p></div></div><div className="admin-hero-status"><span className="admin-live-dot" /><div><strong>Administrator · Full access</strong><small>No activation fee. Protected actions are checked by Supabase.</small></div></div></section>
    ...
    {loading ? <LoadingCards count={5} /> : error ? <ErrorState message={t("common.error")} /> : <>
      {tab === "dashboard" && stats && <div className="admin-dashboard"><div className="admin-access-note"><ShieldCheck size={22} /><div><strong>This Admin account never pays an activation fee.</strong><span>You have full protected access and can control customer access, money, jobs, proofs, services, content and site settings here.</span></div></div><div className="admin-command-grid">
        ...
        <button className="admin-command-card" onClick={() => { window.location.href = "/admin/payments"; }}><span className="admin-command-icon"><CreditCard size={21} /></span><strong>Payment Management</strong><small>Set activation and badge prices, provider & payments</small></button>
      ...
      {tab === "settings" && <form className="card" style={{ padding: 22, display: "grid", gap: 15, marginBottom: 14 }} onSubmit={saveSettings}><h2 className="section-title">Media, payout, support & legal pages</h2><div className="field"><label>Upload logo</label><input type="file" accept="image/jpeg,image/png,image/webp,image/svg+xml" onChange={(event) => setLogoFile(event.target.files?.[0] ?? null)} /></div><div className="field"><label>Favicon URL</label><input className="input" type="url" value={settingsDraft.faviconUrl ?? ""} onChange={(event) => setSettingsDraft((value) => ({ ...value, faviconUrl: event.target.value || null }))} /></div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}><div className="field"><label>Currency</label><input className="input" maxLength={3} value={settingsDraft.currency} onChange={(event) => setSettingsDraft((value) => ({ ...value, currency: event.target.value.toUpperCase().replace(/[^A-Z]/g, "").slice(0,3) }))} /></div><div className="field"><label>Member badge wording</label><input className="input" value={settingsDraft.memberBadgeWording} onChange={(event) => setSettingsDraft((value) => ({ ...value, memberBadgeWording: event.target.value }))} /></div></div><div className="field"><label>Payout methods (comma separated)</label><input className="input" value={settingsDraft.payoutMethods.join(", ")} onChange={(event) => setSettingsDraft((value) => ({ ...value, payoutMethods: event.target.value.split(",").map((item) => item.trim()).filter(Boolean) }))} /></div><div className="field"><label>Support phone</label><input className="input" value={settingsDraft.supportPhone} onChange={(event) => setSettingsDraft((value) => ({ ...value, supportPhone: event.target.value }))} /></div><div className="field"><label>Support icon URL</label><input className="input" type="url" value={settingsDraft.supportIconUrl} onChange={(event) => setSettingsDraft((value) => ({ ...value, supportIconUrl: event.target.value }))} /></div><div className="field"><label>Support position</label><select className="select" value={settingsDraft.supportPosition} onChange={(event) => setSettingsDraft((value) => ({ ...value, supportPosition: event.target.value as "left" | "right" }))}><option value="right">Right</option><option value="left">Left</option></select></div><div className="field"><label>General notice</label><textarea className="textarea" value={settingsDraft.generalNotice} onChange={(event) => setSettingsDraft((value) => ({ ...value, generalNotice: event.target.value }))} /></div><section className="admin-legal-editor"><div className="admin-legal-editor-head"><FileText size={20} /><div><strong>Menu page content</strong><small>These pages appear in the customer menu. Edit English and Bangla independently.</small></div></div><div className="admin-legal-grid"><div className="field"><label>About Us · English</label><textarea className="textarea admin-legal-textarea" maxLength={12000} value={settingsDraft.aboutContent} onChange={(event) => setSettingsDraft((value) => ({ ...value, aboutContent: event.target.value }))} /></div><div className="field"><label>About Us · বাংলা</label><textarea className="textarea admin-legal-textarea" maxLength={12000} value={settingsDraft.aboutContentBn} onChange={(event) => setSettingsDraft((value) => ({ ...value, aboutContentBn: event.target.value }))} /></div><div className="field"><label>Privacy Policy · English</label><textarea className="textarea admin-legal-textarea" maxLength={12000} value={settingsDraft.privacyContent} onChange={(event) => setSettingsDraft((value) => ({ ...value, privacyContent: event.target.value }))} /></div><div className="field"><label>Privacy Policy · বাংলা</label><textarea className="textarea admin-legal-textarea" maxLength={12000} value={settingsDraft.privacyContentBn} onChange={(event) => setSettingsDraft((value) => ({ ...value, privacyContentBn: event.target.value }))} /></div><div className="field"><label>Terms & Conditions · English</label><textarea className="textarea admin-legal-textarea" maxLength={12000} value={settingsDraft.termsContent} onChange={(event) => setSettingsDraft((value) => ({ ...value, termsContent: event.target.value }))} /></div><div className="field"><label>Terms & Conditions · বাংলা</label><textarea className="textarea admin-legal-textarea" maxLength={12000} value={settingsDraft.termsContentBn} onChange={(event) => setSettingsDraft((value) => ({ ...value, termsContentBn: event.target.value }))} /></div></div></section><div className="soft-card" style={{ padding: 12 }}><strong>Payment gateway</strong><div className="muted">{settingsDraft.paymentGatewayStatus === "configured" ? "Configured" : "Not configured"}. Activation is never granted by a client-side click.</div></div><button className="primary-button"><Settings size={18} />{t("admin.save")}</button></form>}
