...
export function ProfilePaymentActions() {
  ...
  return <div aria-label="Profile payment actions" style={{ position: "fixed", right: 16, bottom: 88, zIndex: 35, display: "flex", flexDirection: "column", gap: 8, alignItems: "flex-end" }}>
    {!verified && <Link className="primary-button" href="/payment/checkout?type=verification" style={{ boxShadow: "0 10px 30px rgba(0,0,0,.14)", textDecoration: "none" }}><BadgeCheck size={17} />Get Verified</Link>}
    <Link className="secondary-button" href="/reselling?view=orders" style={{ background: "var(--surface, #fff)", boxShadow: "0 8px 24px rgba(0,0,0,.10)", textDecoration: "none" }}><ShoppingBag size={17} />{language === "bn" ? "অর্ডার / ক্রয়" : "Orders / Purchases"}</Link>
