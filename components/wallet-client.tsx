...
import { ArrowDownToLine, ArrowRightLeft, ArrowUpFromLine, Clock3, Coins, History, Landmark, ReceiptText, TrendingUp, WalletCards } from "lucide-react";
...
export function WalletClient() {
  ...
  useEffect(() => { if (search.get("deposit") === "1") setDepositOpen(true); }, [search]);
  ...
  const withdrawalGross = Number(amount || 0);
  const withdrawalCharge = Number.isFinite(withdrawalGross) ? Math.round(withdrawalGross * 10) / 100 : 0;
  const withdrawalNet = Math.max(0, withdrawalGross - withdrawalCharge);
  ...
  const submitDeposit = async (event: FormEvent) => {
    ...
    if (!Number.isFinite(numericAmount) || numericAmount < 10 || numericAmount > 100000) {
      setMessage({ type: "error", text: language === "bn" ? "ডিপোজিটের পরিমাণ ১০ থেকে ১,০০,০০০ টাকার মধ্যে দিন।" : "Deposit amount must be between 10 and 100,000." });
      return;
    }
    const supabase = getSupabaseBrowserClient(); if (!supabase) return;
    setProcessingDeposit(true); setMessage(null);
    try {
      const { data, error: invokeError } = await supabase.functions.invoke("payment-create", { body: { paymentType: "deposit", amount: numericAmount } });
      if (invokeError) throw invokeError;
      const result = data as { checkoutUrl?: string; error?: string } | null;
      if (!result?.checkoutUrl) throw new Error(result?.error || "Deposit checkout is unavailable.");
      const url = new URL(result.checkoutUrl);
      if (url.protocol !== "https:") throw new Error("invalid_checkout_url");
      window.location.assign(url.toString());
    } catch (cause) {
      setMessage({ type: "error", text: await friendlyPaymentError(cause, language) });
  ...
  const submitWithdrawal = async (event: FormEvent) => {
    ...
    if (requestError) setMessage({ type: "error", text: requestError.message });
      ...
      setMessage({ type: "success", text: language === "bn" ? `উইথড্র রিকোয়েস্ট পাঠানো হয়েছে। ১০% চার্জ ${format(withdrawalCharge)} কেটে আপনি পাবেন ${format(withdrawalNet)}।` : `Withdrawal submitted. After the 10% charge you will receive ${format(withdrawalNet)}.` });
      setAmount(""); setDestination(""); setWithdrawOpen(false); await load();
    }
  };
  ...
  const exchangeCoins = async (event: FormEvent) => {
    ...
    if (exchangeError) { setMessage({ type: "error", text: exchangeError.message }); return; }
  ...
  return <AppShell variant="hub"><main className="custom-wallet-page"><div className="custom-wallet-container">
    ...
    {loading ? <LoadingCards count={4} /> : error || !summary ? <ErrorState message={t("common.error")} /> : <>
      <section className="custom-wallet-balance-card">
        <div className="custom-wallet-balance-inner"><div className="custom-wallet-balance-label">{t("wallet.balance")}</div><div className="custom-wallet-balance-amount">{format(summary.balance)}</div><small>{language === "bn" ? "ওয়েবসাইটের সকল কেনাকাটা ও পেমেন্ট এই মূল ওয়ালেট থেকে হবে।" : "All website purchases and payments use this main wallet."}</small></div>
        <div className="custom-wallet-actions">
          <button className="custom-wallet-btn custom-wallet-btn-primary" onClick={() => { setMessage(null); setDepositOpen(true); }}><ArrowUpFromLine size={18} />{language === "bn" ? "ডিপোজিট" : "Deposit"}</button>
          <button className="custom-wallet-btn custom-wallet-btn-secondary" onClick={() => { setMessage(null); setWithdrawOpen(true); }}><ArrowDownToLine size={18} />{t("wallet.withdraw")}</button>
          <a href="#history" className="custom-wallet-btn custom-wallet-btn-secondary"><History size={18} />{t("wallet.history")}</a>
        </div>
      </section>
      ...
      <section id="history" className="custom-wallet-history"><h2>{t("wallet.history")}</h2>{transactions.length ? <div className="custom-wallet-history-list">{transactions.map((tx) => { const numeric = Number(tx.amount); return <div key={tx.id} className="custom-wallet-history-row"><div className="custom-wallet-history-icon"><WalletCards size={20} /></div><div className="custom-wallet-history-copy"><strong>{tx.description || tx.transaction_type.replaceAll("_", " ")}</strong><span><Clock3 size={12} /> {new Date(tx.created_at).toLocaleString(language === "bn" ? "bn-BD" : "en")}</span></div><strong className={numeric >= 0 ? "credit" : "debit"}>{numeric >= 0 ? "+" : ""}{format(numeric)}</strong></div>; })}</div> : <EmptyState message={t("wallet.empty")} />}</section>
    </>}
  </div></main>

  {depositOpen && <Modal title={language === "bn" ? "ওয়ালেটে ডিপোজিট" : "Deposit to Wallet"} onClose={() => !processingDeposit && setDepositOpen(false)}><form className="auth-form" onSubmit={submitDeposit}><div className="field"><label>{language === "bn" ? "ডিপোজিটের পরিমাণ" : "Deposit amount"}</label><input className="input" type="number" min={10} max={100000} step="0.01" value={depositAmount} onChange={(event) => setDepositAmount(event.target.value)} placeholder="500" required /></div><div className="wallet-exchange-rate">{language === "bn" ? "পেমেন্ট সফলভাবে যাচাই হওয়ার পর সম্পূর্ণ টাকা আপনার মূল Wallet Balance-এ যোগ হবে।" : "After successful verification, the full amount is credited to your main wallet."}</div><button className="primary-button" disabled={processingDeposit}><ArrowUpFromLine size={18} />{processingDeposit ? (language === "bn" ? "চেকআউট খোলা হচ্ছে..." : "Opening checkout...") : (language === "bn" ? "ডিপোজিট করুন" : "Deposit")}</button></form></Modal>}

  {withdrawOpen && <Modal title={t("wallet.withdraw")} onClose={() => setWithdrawOpen(false)}><form className="auth-form" onSubmit={submitWithdrawal}><div className="field"><label>{t("wallet.amount")} · minimum {format(general.withdrawalMinimum)}</label><input className="input" type="number" min={general.withdrawalMinimum} max={summary?.balance ?? undefined} step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} required /></div><div className="wallet-exchange-rate"><strong>{language === "bn" ? "উইথড্র চার্জ: ১০%" : "Withdrawal charge: 10%"}</strong><br />{language === "bn" ? "আপনি অনুরোধ করছেন" : "Requested"}: {format(withdrawalGross)} · {language === "bn" ? "চার্জ" : "Charge"}: {format(withdrawalCharge)} · <strong>{language === "bn" ? "আপনি পাবেন" : "You receive"}: {format(withdrawalNet)}</strong></div><div className="field"><label>{t("wallet.method")}</label><select className="select" value={method} onChange={(event) => setMethod(event.target.value)} required>{general.payoutMethods.map((value) => <option key={value}>{value}</option>)}</select></div><div className="field"><label>{t("wallet.destination")}</label><div className="input-wrap"><Landmark size={19} /><input className="input with-icon" value={destination} onChange={(event) => setDestination(event.target.value)} required maxLength={120} /></div></div><button className="primary-button" disabled={withdrawalGross <= 0 || withdrawalGross > Number(summary?.balance ?? 0)}>{t("wallet.request")}</button></form></Modal>}
