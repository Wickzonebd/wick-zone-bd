  ...
  return <AppShell variant="hub"><main className="custom-wallet-page"><div className="custom-wallet-container">
    ...
    {loading ? <LoadingCards count={3} /> : error || !summary ? <ErrorState message={t("common.error")} /> : <>
      <section className="custom-wallet-balance-card">
        <div className="custom-wallet-balance-inner">
          <div className="custom-wallet-balance-label">{t("wallet.balance")}</div>
          <div className="custom-wallet-balance-amount">{format(summary.balance)}</div>
        </div>
        <div className="custom-wallet-actions">
          <button className="custom-wallet-btn custom-wallet-btn-primary" onClick={() => setWithdrawOpen(true)}><ArrowDownToLine size={18} />{t("wallet.withdraw")}</button>
          <a href="#history" className="custom-wallet-btn custom-wallet-btn-secondary"><History size={18} />{t("wallet.history")}</a>
        </div>
      </section>

      <section className="custom-wallet-stats-grid">{periods.map(([label, value]) => <div className="custom-wallet-stat-card" key={label}>
        <div className="custom-wallet-stat-icon"><ReceiptText size={20} /></div>
        <div className="custom-wallet-stat-amount">{format(value)}</div>
        <div className="custom-wallet-stat-label">{t(label)}</div>
      </div>)}</section>

      <section id="history" className="custom-wallet-history">
        <h2>{t("wallet.history")}</h2>
        {transactions.length ? <div className="custom-wallet-history-list">{transactions.map((tx) => <div key={tx.id} className="custom-wallet-history-row">
          <div className="custom-wallet-history-icon"><WalletCards size={20} /></div>
          <div className="custom-wallet-history-copy"><strong>{tx.description || tx.transaction_type.replaceAll("_", " ")}</strong><span><Clock3 size={12} /> {new Date(tx.created_at).toLocaleString()}</span></div>
          <strong className={tx.amount >= 0 ? "credit" : "debit"}>{tx.amount >= 0 ? "+" : ""}{format(tx.amount)}</strong>
        </div>)}</div> : <EmptyState message={t("wallet.empty")} />}
      </section>
    </>}
  </div></main>
