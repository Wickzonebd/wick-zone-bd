...
export function AdminResellingManager({ currency }: { currency: string }) {
  ...
  const reset = () => {
    setProductDraft({ ...emptyProduct }); setProductImageFile(null); setCategoryDraft({ ...emptyCategory }); setCategoryImageFile(null); setVendorDraft({ ...emptyVendor }); setVendorLogoFile(null); setBannerDraft({ ...emptyBanner }); setBannerImageFile(null); setEditingId(null);
  };
  ...
  const saveCategory = async (event: FormEvent) => {
    ...
    if (categoryImageFile && (!["image/jpeg", "image/png", "image/webp"].includes(categoryImageFile.type) || categoryImageFile.size > 8 * 1024 * 1024)) { setMessage({ type: "error", text: "Category image must be JPG, PNG or WebP and no larger than 8 MB." }); return; }
    const supabase = getSupabaseBrowserClient(); if (!supabase) return; setSaving(true); setMessage(null);
    let imageUrl = categoryDraft.imageUrl.trim() || null; let uploadedPath: string | null = null;
    if (categoryImageFile) {
      uploadedPath = `reselling/categories/${Date.now()}-${safeFileName(categoryImageFile.name)}`;
      const upload = await supabase.storage.from("job-media").upload(uploadedPath, categoryImageFile, { contentType: categoryImageFile.type, upsert: false });
      if (upload.error) { setSaving(false); setMessage({ type: "error", text: upload.error.message }); return; }
      imageUrl = supabase.storage.from("job-media").getPublicUrl(uploadedPath).data.publicUrl;
    }
    const payload = { name_en: categoryDraft.nameEn.trim(), name_bn: categoryDraft.nameBn.trim() || null, image_url: imageUrl, sort_order: sort, is_active: categoryDraft.isActive };
  ...
  const saveVendor = async (event: FormEvent) => {
    ...
    if (vendorLogoFile && (!["image/jpeg", "image/png", "image/webp"].includes(vendorLogoFile.type) || vendorLogoFile.size > 8 * 1024 * 1024)) { setMessage({ type: "error", text: "Vendor logo must be JPG, PNG or WebP and no larger than 8 MB." }); return; }
    const supabase = getSupabaseBrowserClient(); if (!supabase) return; setSaving(true); setMessage(null);
    let logoUrl = vendorDraft.logoUrl.trim() || null; let uploadedPath: string | null = null;
    if (vendorLogoFile) {
      uploadedPath = `reselling/vendors/${Date.now()}-${safeFileName(vendorLogoFile.name)}`;
      const upload = await supabase.storage.from("job-media").upload(uploadedPath, vendorLogoFile, { contentType: vendorLogoFile.type, upsert: false });
      if (upload.error) { setSaving(false); setMessage({ type: "error", text: upload.error.message }); return; }
      logoUrl = supabase.storage.from("job-media").getPublicUrl(uploadedPath).data.publicUrl;
    }
    const payload = { name: vendorDraft.name.trim(), description: vendorDraft.description.trim() || null, logo_url: logoUrl, website_url: vendorDraft.websiteUrl.trim() || null, sort_order: sort, is_active: vendorDraft.isActive };
  ...
  const saveBanner = async (event: FormEvent) => {
    ...
    if (bannerImageFile && (!["image/jpeg", "image/png", "image/webp"].includes(bannerImageFile.type) || bannerImageFile.size > 8 * 1024 * 1024)) { setMessage({ type: "error", text: "Banner image must be JPG, PNG or WebP and no larger than 8 MB." }); return; }
    const supabase = getSupabaseBrowserClient(); if (!supabase) return; setSaving(true); setMessage(null);
    let imageUrl = bannerDraft.imageUrl.trim() || null; let uploadedPath: string | null = null;
    if (bannerImageFile) {
      uploadedPath = `reselling/banners/${Date.now()}-${safeFileName(bannerImageFile.name)}`;
      const upload = await supabase.storage.from("job-media").upload(uploadedPath, bannerImageFile, { contentType: bannerImageFile.type, upsert: false });
      if (upload.error) { setSaving(false); setMessage({ type: "error", text: upload.error.message }); return; }
      imageUrl = supabase.storage.from("job-media").getPublicUrl(uploadedPath).data.publicUrl;
    }
    const payload = { title_en: bannerDraft.titleEn.trim(), title_bn: bannerDraft.titleBn.trim() || null, subtitle_en: bannerDraft.subtitleEn.trim() || null, subtitle_bn: bannerDraft.subtitleBn.trim() || null, image_url: imageUrl, destination_url: bannerDraft.destinationUrl.trim() || null, sort_order: sort, is_active: bannerDraft.isActive };
  ...
  const editCategory = (item: Category) => { setEditingId(item.id); setCategoryImageFile(null); setCategoryDraft({ nameEn: item.name_en, nameBn: item.name_bn ?? "", imageUrl: item.image_url ?? "", sortOrder: String(item.sort_order), isActive: item.is_active }); window.scrollTo({ top: 0, behavior: "smooth" }); };
  const editVendor = (item: Vendor) => { setEditingId(item.id); setVendorLogoFile(null); setVendorDraft({ name: item.name, description: item.description ?? "", logoUrl: item.logo_url ?? "", websiteUrl: item.website_url ?? "", sortOrder: String(item.sort_order), isActive: item.is_active }); window.scrollTo({ top: 0, behavior: "smooth" }); };
  const editBanner = (item: Banner) => { setEditingId(item.id); setBannerImageFile(null); setBannerDraft({ titleEn: item.title_en, titleBn: item.title_bn ?? "", subtitleEn: item.subtitle_en ?? "", subtitleBn: item.subtitle_bn ?? "", imageUrl: item.image_url ?? "", destinationUrl: item.destination_url ?? "", sortOrder: String(item.sort_order), isActive: item.is_active }); window.scrollTo({ top: 0, behavior: "smooth" }); };
