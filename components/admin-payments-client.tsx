...
export function AdminPaymentsClient(){
  ...
  const load=useCallback(async()=>{if(!isAdmin){setLoading(false);return;}const supabase=getSupabaseBrowserClient();if(!supabase){setLoading(false);return;}setLoading(true);const [p,s]=await Promise.all([
    ...
    supabase.from("payment_settings").select("payment_enabled,provider_name,currency,merchant_name,merchant_logo,support_phone,support_email,payment_success_message,payment_failed_message,invoice_footer,terms_text,micro_job_activation_price,verification_price,verification_enabled,verification_duration_days").eq("id",true).maybeSingle()
  ...
  return <AppShell hidePrimaryNav><main className="payment-page"><div className="payment-admin-container">
    ...
    {settings&&<form id="payment-settings" className="payment-admin-panel payment-settings-form" onSubmit={save}>
      <div className="payment-settings-title"><Settings size={20}/><div><h2>Service Pricing & Payment Settings</h2><p>Set customer prices and payment availability. Provider credentials always remain server-side.</p></div></div>
      <div className="payment-api-status"><CreditCard size={18}/><strong>Hosted checkout · customers choose an available payment method on the secure provider page.</strong></div>
      ...
      <div className="payment-api-status"><ShieldCheck size={18}/><strong>Hosted payment API: {settings.provider_name?"Configured · credential hidden":"Not configured"}</strong></div>
